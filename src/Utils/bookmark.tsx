...
export const getBookmarks = (userId: any) => {
  ...
  if (userId !== null && userId !== undefined) {
    ...
    const values: any = localStorage.getItem("VelzorixStreamWatchlist");
...
export const setBookmarks = ({ userId = null, type, id }: any) => {
  if (userId !== null) {
    ...
    if (!values[type]?.includes(id)) {
      ...
      values[type]?.push(id);
      values[type] = values[type].reverse();
      localStorage.setItem("VelzorixStreamWatchlist", JSON.stringify(values));
    }
  }
};
